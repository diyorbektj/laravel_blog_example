...
<div class="bg-white w-full h-auto">
   ...
   @php
       $message = \App\Models\ChatMessages::where('chat_id',$item->id)->orderByDesc('id')->get();
   @endphp
   <a href="/chat/index/{{$item->id}}#{{$message[0]->id}}">
   
      <div class="flex justify-between">
       <div>
           <div class="p-2 flex items-center" style="position: relative;">
               <div>
                   <img class="w-20 h-20 rounded-full border-4 border-green-300" src="{{$item->image}}" alt="">
               </div>
               <div class="pl-4" style="align-items: none;">
                   ...
                   <div class="text-gray-400 flex items-center">
                    <p class="font-bold pr-2">{{$message[0]->user->name}}:</p>
                      <p>{{Str::limit($message[0]->message, 100);}}</p>
                   </div>
               </div>
           </div>
       </div>
